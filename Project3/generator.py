...
class Node:
    def __init__(self, isHead, headID, inClass, inID, isSwitch = False):
        ...
        # max number of edges I had (before being used in noc.bsv generation)
        self.maxEdges = 0
...
print('Done.')
N_STEPS = 5
S_STEPS = [ "Adding instantiation of L1 (Head Router) Nodes",
            "Adding edges between L1 (Head Router) Nodes",
            "Adding instantiation of L2 Nodes",
            "Adding edges between L2 Nodes",
            "Adding instantiation of cores; linking cores and routers to nodes"]

def print_step_info(index):
  tmpOUT = f'\n\n // [{index}/{N_STEPS}] {S_STEPS[index-1]} \n\n'
  print(f'[{index}/{N_STEPS}] {S_STEPS[index - 1]}')
  return tmpOUT
...
for node in G.nodes:
    ...
    if tc == 'R':
        ...
        OUT += f'        Ifc_node#({ec}) {nodeId} <- chain_l2({ec}, Node_addr {{l1_headID: {int(node)}, l2_ID: 999}}, {L1_n}, 1, 2, False, True);\n'
    elif tc == 'M':
        headIdMode = 1
    ...
    #     OUT += f'Ifc_node {nodeId} <- butterfly_l2({}, {{l1_headID: {}, l2_ID: {}}}, {L1_n}, {L1_m}, 1, 2, 3, 4, False, True);\n''
        ...
        nodeIntId = [int(i) for i in node.split(DELIMITER)]
        nodeIntId = nodeIntId[0] * L1_n + nodeIntId[1]
        OUT += f'        Ifc_node#({ec}) {nodeId} <- folrus_l2({ec}, Node_addr {{l1_headID: {nodeIntId}, l2_ID: 999}}, {L1_n}, {L1_m}, 1, 2, 3, 4, False, True);\n'
    elif tc == 'H':
        OUT += f'        Ifc_node#({ec}) {nodeId} <- hypercube_l2({ec}, Node_addr {{l1_headID: {int(node)}, l2_ID: 999}}, 1, 2, 3, False, True);\n'
    else:
        print('Unknown topology provided in L1Topology.txt: ' + tc)
        exit()

OUT += print_step_info(2)
...
for head in innerTopologies:
    ...
    for node in G.nodes:
        ...
        if tc == 'R':
            ...
            OUT += f'        Ifc_node#({ec}) {nodeId} <- chain_l2({ec}, Node_addr {{l1_headID: {int(headIntId)}, l2_ID: {int(nodeIntId)}}}, {L1_n}, 1, 2, {G.nodes[node]["exdata"].isHead}, False);\n'
        elif tc == 'M':
        ...
        #     OUT += f'Ifc_node {nodeId} <- butterfly_l2({}, {{l1_headID: {}, l2_ID: {}}}, {L1_n}, {L1_m}, 1, 2, 3, 4, False, True);\n''
            ...
            nodeIntId = [int(i) for i in nodeIntId.split(DELIMITER)]
            nodeIntId = nodeIntId[0] * T.topoParams[0] + nodeIntId[1]
            OUT += f'        Ifc_node#({ec}) {nodeId} <- folrus_l2({ec}, Node_addr {{l1_headID: {int(headIntId)}, l2_ID: {int(nodeIntId)}}}, {L1_n}, {L1_m}, 1, 2, 3, 4, {G.nodes[node]["exdata"].isHead}, False);\n'
        elif tc == 'H':
            OUT += f'        Ifc_node#({ec}) {nodeId} <- hypercube_l2({ec}, Node_addr {{l1_headID: {int(headIntId)}, l2_ID: {int(nodeIntId)}}}, 1, 2, 3, {G.nodes[node]["exdata"].isHead}, False);\n'
        else:
            print('Unknown topology provided in L1Topology.txt: ' + tc)
            exit()

OUT += print_step_info(4)
